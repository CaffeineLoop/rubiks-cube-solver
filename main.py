...
Supports LBL solving method with comprehensive features
"""
...
def main_menu():
    """Display enhanced main menu with LBL support"""
    ...
    while True:
        ...
        print("5. Auto-solve cube (Layer-by-Layer)")
        ...
        if choice == '1':
            ...
            solve_cube(cube, current_size, "LBL")  # Same as option 4
        elif choice == '6':
...
def solve_cube(cube, size, method):
    ...
    print(f"\n=== Starting Layer-by-Layer Auto-Solver for {size}x{size}x{size} Cube ===")
    if size != 3:
        print(f"Warning: Solver optimized for 3x3 cubes. {size}x{size}x{size} support is experimental.")
    
    solver = RubiksCubeSolver(cube)  # ✅ Fixed initialization
    ...
    print(f"\nSolver Statistics for {size}x{size}x{size} cube:")
    print(f"Total moves attempted: {stats['total_moves']}")
    print(f"Successfully solved: {stats['is_solved']}")
    print(f"Solution sequence: {stats['solution_sequence']}")
    ...
    if solver.moves:
        ...
        for move in solver.moves:
            move_breakdown[move] = move_breakdown.get(move, 0) + 1
        print(f"Move breakdown:")
        for move, count in sorted(move_breakdown.items()):
...
def complete_demo(size, method):
    ...
    print(f"\n=== Complete {size}x{size}x{size} Rubik's Cube Demo (Layer-by-Layer) ===")
    ...
    print(f"\n3. Attempting to solve {size}x{size}x{size} cube using Layer-by-Layer method...")
    solver = RubiksCubeSolver(demo_cube)  # ✅ Fixed initialization
    solution = solver.solve()
    ...
    stats = solver.get_solving_statistics()
    print(f"\nDemo completed for {size}x{size}x{size} cube!")
...
def advanced_solver_statistics(cube, size, method):
    ...
    print(f"  Algorithms: Layer-by-Layer")
    print(f"  Features: Move tracking, state detection")
    ...
    print(f"  ✓ Layer-by-Layer solving method")
